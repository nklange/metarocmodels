...
parameters {
  ...
  real<lower=0> sigma_cr[nthres - 1];
...
model {
  ...
  if (i < (nthres + 1) %/% 2) Intercept[,i] ~ normal(mu_cr[i], sigma_cr[i]);
  if (i == (nthres + 1) %/% 2)  Intercept[,i] ~ normal(mu_cr[i], .1);
  if (i > (nthres + 1) %/% 2) Intercept[,i] ~ normal(mu_cr[i], sigma_cr[i-1]);
