...
parameters {
  vector[K] b;  // population-level effects on mu_o (for forward,backward,random group)
...
transformed parameters {
  ...
  vector[N_1] r_1_1;
  vector[N_1] r_1_2;
  vector[N_1] r_1_disc_3;
  vector[N_1] r_1_disc_4;
...
model {
  ...
  if (!prior_only) {
    ...
    for (n in 1:N) {
      // add more terms to the linear predictor
      disc[n] += r_1_disc_3[J_1[n]] * Z_1_disc_3[n] + r_1_disc_4[J_1[n]] * Z_1_disc_4[n];
    }
    for (n in 1:N) {
