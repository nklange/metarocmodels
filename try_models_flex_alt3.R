...
model_formula_ws2 <- bf(rating ~  isold:condition + (isold:condition|s|id) + (1|item) ,
...
stanmodelws2 <- make_stancode(model_formula_ws2,
...
tmpdat_ws2 <- make_standata(model_formula_ws2,
                        family = cumulative(link = "probit"),
                        data = alt3exp)
#str(tmpdat)
...
fit_ws2$save_object(file = "Fits/fit_uvsdt_e4_v1.RDS")
saveRDS(rstan::read_stan_csv(fit_ws2$output_files()),"rstanfits/fit_uvsdt_e4_v1_rstan.rds")
